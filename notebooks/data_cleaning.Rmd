...
## *Note: Spline for rep2-4 seems to overshoot the fit. It does reduce the SE(meanF) for isogenic controls but predicts functional controls as null.
...
rep4 <- meanF_data %>% filter(Count_total_REP4 >= rc ) %>%
  dplyr::select(AA_var,REBC,bg,RE,meanF_REP4,type) %>% dplyr::rename(meanF = meanF_REP4) %>% mutate(REP = "REP4") # all rep4 variants
rep4_ctls <- meanF_data %>% filter(Count_total_REP4 >= rc & type== "control") %>%
  filter(AA_var %in% c("GSKV","EGKA","GGKM")) %>% dplyr::select(AA_var,REBC,bg,RE,meanF_REP4,type) %>% dplyr::rename(meanF = meanF_REP4) %>%
  mutate(REP = "REP4")
