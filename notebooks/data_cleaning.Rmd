...
if(!file.exists(file.path("..", "data", "meanF", "meanF_data.rda"))) {
  ...
  meanF_data <- rbind(meanF_data$rep1, meanF_data$rep2, meanF_data$rep3, meanF_data$rep4) %>% 
    select(AA_var,REBC,bg,RE,REP,meanF,Count_total,cellCount_total) %>% 
...
Use splines to correct *all* variants that passed the read count filter - not only the ones that are shared with rep2 and the other replicates. This will also reduce the SE(meanF) of variants that were filtered out before.
