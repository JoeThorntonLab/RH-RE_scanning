...
DMS binned sort data is filtered based on two approaches. First, we filter out variants with low read count (across replicates) to reduce the global standard error; second, we remove variants with high SE(meanF) to further reduce the standard error. 
...
After having identified the read threshold, we will perform a second filter removing variants with SE(meanF) > 0.1. This reduces the noise, and will help to fit the splines in order to capture the global relationships between replicates.
...
Here we will correct for the remaining non linearity between replicates in order to adjust all measurements to be on the same scale (the reference scale being replicate 2). 
...
We'll use the fitted splines to correct *all* variants that passed the read count filter - not only the ones that are shared with rep2 and the other replicates. This will also reduce the SE(meanF) of variants that were filtered out before.
...
=======
rep2 <- meanF_data %>% filter(Count_total_REP2 >= rc) %>%
  select(AA_var,REBC,bg,RE,meanF_REP2,type,cellCount_total_REP1,cellCount_total_REP2,cellCount_total_REP3,cellCount_total_REP4,
         cellCount_b1_REP1,cellCount_b2_REP1,cellCount_b3_REP1,cellCount_b4_REP1,cellCount_b1_REP2,cellCount_b2_REP2,cellCount_b3_REP2,cellCount_b4_REP2,
         cellCount_b1_REP3,cellCount_b2_REP3,cellCount_b3_REP3,cellCount_b4_REP3,cellCount_b1_REP4,cellCount_b2_REP4,cellCount_b3_REP4,cellCount_b4_REP4) %>% 
  rename(meanF = meanF_REP2) %>% mutate(REP = "REP2") # all rep2 variants
rep1 <- meanF_data %>% filter(Count_total_REP1 >= rc) %>%
  select(AA_var,REBC,bg,RE,meanF_REP1,type,cellCount_total_REP1,cellCount_total_REP2,cellCount_total_REP3,cellCount_total_REP4,
         cellCount_b1_REP1,cellCount_b2_REP1,cellCount_b3_REP1,cellCount_b4_REP1,cellCount_b1_REP2,cellCount_b2_REP2,cellCount_b3_REP2,cellCount_b4_REP2,
         cellCount_b1_REP3,cellCount_b2_REP3,cellCount_b3_REP3,cellCount_b4_REP3,cellCount_b1_REP4,cellCount_b2_REP4,cellCount_b3_REP4,cellCount_b4_REP4) %>%
  mutate(REP = "REP1") # all rep1 variants
rep3 <- meanF_data %>% filter(Count_total_REP3 >= rc) %>%
  select(AA_var,REBC,bg,RE,meanF_REP3,type,cellCount_total_REP1,cellCount_total_REP2,cellCount_total_REP3,cellCount_total_REP4,
         cellCount_b1_REP1,cellCount_b2_REP1,cellCount_b3_REP1,cellCount_b4_REP1,cellCount_b1_REP2,cellCount_b2_REP2,cellCount_b3_REP2,cellCount_b4_REP2,
         cellCount_b1_REP3,cellCount_b2_REP3,cellCount_b3_REP3,cellCount_b4_REP3,cellCount_b1_REP4,cellCount_b2_REP4,cellCount_b3_REP4,cellCount_b4_REP4) %>%
  mutate(REP = "REP3") # all rep3 variants
rep4 <- meanF_data %>% filter(Count_total_REP4 >= rc ) %>%
  select(AA_var,REBC,bg,RE,meanF_REP4,type,cellCount_total_REP1,cellCount_total_REP2,cellCount_total_REP3,cellCount_total_REP4,
         cellCount_b1_REP1,cellCount_b2_REP1,cellCount_b3_REP1,cellCount_b4_REP1,cellCount_b1_REP2,cellCount_b2_REP2,cellCount_b3_REP2,cellCount_b4_REP2,
         cellCount_b1_REP3,cellCount_b2_REP3,cellCount_b3_REP3,cellCount_b4_REP3,cellCount_b1_REP4,cellCount_b2_REP4,cellCount_b3_REP4,cellCount_b4_REP4) %>%
  rename(meanF = meanF_REP4) %>% mutate(REP = "REP4") # all rep4 variants
rep4_ctls <- meanF_data %>% filter(Count_total_REP4 >= rc & type== "control") %>%
  filter(AA_var %in% c("GSKV","EGKA","GGKM")) %>%
  select(AA_var,REBC,bg,RE,meanF_REP4,type,cellCount_total_REP1,cellCount_total_REP2,cellCount_total_REP3,cellCount_total_REP4,
         cellCount_b1_REP1,cellCount_b2_REP1,cellCount_b3_REP1,cellCount_b4_REP1,cellCount_b1_REP2,cellCount_b2_REP2,cellCount_b3_REP2,cellCount_b4_REP2,
         cellCount_b1_REP3,cellCount_b2_REP3,cellCount_b3_REP3,cellCount_b4_REP3,cellCount_b1_REP4,cellCount_b2_REP4,cellCount_b3_REP4,cellCount_b4_REP4) %>%
  rename(meanF = meanF_REP4) %>% mutate(REP = "REP4")
>>>>>>> not much modified here
...
Now let's turn to filtering the data from the debulk sort (first round of sorting to enrich for GFP+ variants). For this experiment, we had two sort bins: GFP- and GFP+. We sequenced cells collected in the GFP- bin to determine the set of variants that were present in the unsorted library. Variants that are seen in the debulk GFP- dataset but not in the binned sort (round 2 sort) dataset may be null variants that did not have a high enough proportion of GFP+ cells to be collected in the GFP+ bin during the debulk sort. Alternatively, they may be variants that are active (i.e. they have a significant proportion of GFP+ cells), but by chance were not sorted at high enough frequency to be detected in the binned sort dataset. 

We want to have some criterion to call variants null if they are detected in the debulk GFP- datset but not the binned sort dataset. We can use read count as a metric for the likelihood of being null, since variants that are detected at high frequency in the debulk GPF- dataset but not at all in the binned sort dataset are more likely to be null than those that are detected at low frequency in the debulk GFP- dataset. 

How should we determine a read count threshold for calling a variant null in the debulk GPF- dataset? To do this, we can calculate the binomial sampling probability of a variant being sorted into the debulk GFP+ bin given a total number of cells sorted for that variant and the proportion of cells for the variant that are in the GFP+ bin. We can then set a read count threshold to ensure that variants at that read count level that are truly active would have a high probability of being detected in the binned sort data, and conversely if they are not detected in the binned sort data then they are likely to be null.

First, we need to estimate how many cells must be sorted for a variant into the debulk GFP+ bin to be detected in the binned sort dataset (and make it past the binned sort filtering procedures). To do this, we can estimate the average number of cells that were sorted for each variant in the debulk GFP+ bin. We will assume that the proportion of cells per variant per RE library in the binned sort data is the same as the proportion of cells per variant per RE library in the debulk sort GFP+ bin. We can then multiply this by the total number of cells sorted per RE library in the debulk sort to estimate the number of cells sorted into the GFP+ bin ber library.
...
debulkposcells <- data.frame(bg = c(rep(rep(c("AncSR1", "AncSR2"), each = 16), 3), rep("AncSR1", 8)),
                          ...
                          REP = c(rep(c("REP1", "REP2", "REP3"), each = 32), rep("REP4", 8)),
...
debulkposcells <- debulkposcells %>% spread(REP, GFPposcellslib)
