...
## *Note: Spline for rep2-4 seems to overshoot the fit. But it does reduce the SE(meanF) for isogenic controls.

```


```{r plotsplinefits, echo=FALSE}
...
Use splines to correct *all* variants that passed the read count filter in reps 1, 3 and 4 - not only the ones that are shared with rep2. This will also reduce the SE(meanF) of variants that were filtered out before.

```{r correction}
# Variants per replicate that passed initial read count QC filter
rep2 <- meanF_data %>% filter(Count_total_REP2 >= rc) %>%
  dplyr::select(AA_var,REBC,bg,RE,meanF_REP2,type) %>% dplyr::rename(meanF = meanF_REP2) %>% mutate(REP = "REP2") # all rep2 variants
rep1 <- meanF_data %>% filter(Count_total_REP1 >= rc) %>%
  dplyr::select(AA_var,REBC,bg,RE,meanF_REP1,type) %>% mutate(REP = "REP1") # all rep1 variants
rep3 <- meanF_data %>% filter(Count_total_REP3 >= rc) %>%
  dplyr::select(AA_var,REBC,bg,RE,meanF_REP3,type) %>% mutate(REP = "REP3") # all rep3 variants
rep4 <- meanF_data %>% filter(Count_total_REP4 >= rc ) %>%
  dplyr::select(AA_var,REBC,bg,RE,meanF_REP4,type) %>% mutate(REP = "REP4") # all rep4 variants
rep4_ctls <- meanF_data %>% filter(Count_total_REP4 >= rc & type== "control") %>%
  filter(AA_var %in% c("GSKV","EGKA","GGKM")) %>% dplyr::select(AA_var,REBC,bg,RE,meanF_REP4,type) %>% mutate(REP = "REP4")
...
rep1 <- rep1 %>% mutate(meanF_REP1_c = ((predict(basis_mat12, newx = rep1$meanF_REP1) %*% coef(spline_r12)[-1]) + coef(spline_r12)[1])[,1]) %>% dplyr::select(AA_var,REBC,bg,RE,meanF_REP1_c,type,REP) %>% dplyr::rename(meanF = meanF_REP1_c) 

rep3 <- rep3 %>% mutate(meanF_REP3_c = ((predict(basis_mat23, newx = rep3$meanF_REP3) %*% coef(spline_r23)[-1]) + coef(spline_r23)[1])[,1]) %>% dplyr::select(AA_var,REBC,bg,RE,meanF_REP3_c,type,REP) %>% dplyr::rename(meanF = meanF_REP3_c) 

rep4 <- rep4 %>% mutate(meanF_REP4_c = ((predict(basis_mat24, newx = rep4$meanF_REP4) %*% coef(spline_r24)[-1]) + coef(spline_r24)[1])[,1]) %>% dplyr::select(AA_var,REBC,bg,RE,meanF_REP4_c,type,REP) %>% dplyr::rename(meanF = meanF_REP4_c) 
...
meanF_data_corrected %>% filter(n_reps != 1) %>% 
  ...
  theme_classic() + xlab("SE(meanF)") + ylab("Percent of variants") + 
  geom_vline(xintercept = 0.1,col="gray") + scale_y_continuous(labels = scales::percent) +
  annotate(geom = "text", x = 0.35, y = 0.04, label="Removed variants: 1.42%") # histogram of SE(meanF)
...
write.csv(meanF_data_corrected,file=gzfile("meanF_data_corrected_NovaSeq.csv.gz")) # Save new corrected dataset
```
...
ctl_rep1 <- ctls %>% filter(!is.na(REP1)) %>% dplyr::select(AA_var,REBC,REP1) %>% mutate(REP = "REP1") %>% dplyr::rename(meanF = REP1)
ctl_rep4 <- ctls %>% filter(!is.na(REP4)) %>% dplyr::select(AA_var,REBC,REP4) %>% mutate(REP = "REP4") %>% dplyr::rename(meanF = REP4) 
