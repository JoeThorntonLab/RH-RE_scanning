...
if(!dir.exists(file.path("..", "results", "cleaned_data"))) {
  dir.create(file.path("..", "results", "cleaned_data"))
}
```
...
Our estimated number of cells in the GFP+ bin for variants at the fluorescence significance threshold is 8.15.
...
Now let's use our estimates for number and proportion of cells in the GFP+ bin for variants at the fluorescence significance threshold to calculate how many cells we would need to sort for these variants to have >0.95 probability of detecting them as significantly fluorescent in the binned sort experiment. We'll also calculate a corresponding read threshold for the GFP- data given the mean number of reads per cell in that dataset.

```{r calculateGFPnegrcthreshold}
# calculate number of cells we would need to sort to have >=0.95 chance of
# detecting variants at the fluorescence significance threshold in the binned
# sort data
for(i in 1:200) {
  p <- pbinom(round(GFPpluscellssig) - 1, i, proppossig)
  if(p <= 0.05) {  # p<0.05 significance level
    debulk_cc_p <- i  # total number of cells sorted
    print(paste("p < 0.05 total cells =", i))  
    break
  }
}

# do the same but with a Bonferroni correction
for(i in 1:200) {
  p <- pbinom(round(GFPpluscellssig) - 1, i, proppossig)
  if(p <= 0.05 / (21^4*32)) {  # Bonferroni-corrected significance level
    debulk_cc_padj <- i  # total number of cells sorted
    print(paste("p < 8e-9 total cells =", i))  
    break
  }
}

# calculate number of cells in the GFP- bin corresponding to total cell count
# thresholds calculated above
debulk_ccneg_p <- round((1 - proppossig) * debulk_cc_p)
print(paste("p < 0.05 GFP- cells =", debulk_ccneg_p))
debulk_ccneg_padj <- round((1 - proppossig) * debulk_cc_padj)
print(paste("p < 8e-9 GFP- cells =", debulk_ccneg_padj))

### calculate mean reads per cell in the debulk GFP- data
...
# calculate mean reads per cell across debulk libraries
debulknegreadspercell <- debulknegcells %>%
  ungroup() %>%
  summarize(weighted.mean(total_reads/GFPnegcellslib, GFPnegcellslib/sum(GFPnegcellslib))) %>%
  as.numeric()
print(paste("GFP- reads per cell:", debulknegreadspercell))

### calculate number of GFP- reads per variant corresponding to minimum number of
### GFP- cells per variant calculated above
debulk_rcneg_p <- debulk_ccneg_p * debulknegreadspercell
print(paste("p < 0.05 GFP- reads =", debulk_rcneg_p))
debulk_rcneg_padj <- debulk_ccneg_padj * debulknegreadspercell
print(paste("p < 8e-9 GFP- reads =", debulk_rcneg_padj))
```

We would need to sample at least 55 total cells per variant for p < 0.05 (140 for p < 8e-9; Bonferroni corrected cutoff); this translates to 42 (108) cells per variant in the GFP- bin. Given the mean number of reads per cell is 0.86 for the debulk GFP- data, this corresponds to 36 (93) reads per variant. We can infer that variants that have at least this many reads in the debulk GFP- data have a high chance of being measured as fluorescent in the binned sort, and therefore if they are not detected in the binned sort dataset then they likely have null fluorescence.


Compute p-value for each variant

```{r}
debulk_data <- debulk_data %>%
  mutate(cells_GFPneg = Count * 1/)
```




Now let's filter the variants in the debulk GFP- data given this read count threshold.

```{r filterdebulkdata}
# plot read count distribution variants in debulk GFP- data and read count threshold
debulk_data %>%
  ggplot(aes(x = Count)) +
  geom_histogram(bins = 100) +
  geom_vline(xintercept = debulk_rcneg_p, color = "red") +
  geom_vline(xintercept = debulk_rcneg_padj, color = "red", linetype = "dashed") +
  scale_x_log10() +
...
# filter variants at alpha = 0.05 level
debulk_data_filter_p <- debulk_data %>% filter(Count >= debulk_rcneg_p)

# fraction of variants in the debulk GFP- data remaining
debulk_data_filter_p %>% summarize(n_distinct(AA_var, REBC)) /
  debulk_data %>% summarize(n_distinct(AA_var, REBC))

# number of variants remaining (including those in binned sort data)
debulk_data_filter_p %>% 
  select(AA_var, REBC) %>%
  full_join(meanF_data_corrected %>% filter(type == "exp") %>% select(AA_var, REBC)) %>%
  summarize(n_distinct(AA_var, REBC))

# fraction of variants remaining out of all possible variants
debulk_data_filter_p %>% 
  select(AA_var, REBC) %>%
  full_join(meanF_data_corrected %>% filter(type == "exp") %>% select(AA_var, REBC)) %>%
  summarize(n_distinct(AA_var, REBC)) / (21^4*32)

# filter variants at alpha = 8e-9 level
debulk_data_filter_padj <- debulk_data %>% filter(Count >= debulk_rcneg_padj)

# fraction of variants in the debulk GFP- data remaining
debulk_data_filter_padj %>% summarize(n_distinct(AA_var, REBC)) /
  debulk_data %>% summarize(n_distinct(AA_var, REBC))

# number of variants remaining (including those in binned sort data)
debulk_data_filter_padj %>% 
  select(AA_var, REBC) %>%
  full_join(meanF_data_corrected %>% filter(type == "exp") %>% select(AA_var, REBC)) %>%
  summarize(n_distinct(AA_var, REBC))
...
write.csv(debulk_data_filter_p,
          file = file.path("..", "results", "cleaned_data", "debulk_data_filtered.csv"))
```
