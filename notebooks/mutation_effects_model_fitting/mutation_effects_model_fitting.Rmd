...
We see that the fits look quite good. The $R^2$ is again quite low for the AncSR1 dataset at 0.31, but much higher for the subset of active variants at 0.96. The $R^2$ for the AncSR2 dataset is higher at 0.88 for all variants and 0.99 for active variants. Let's compare to the $R^2$ between replicates in the training data.

```{r replicater2, purl = FALSE}
# AncSR1
...
AncSR1_complete_data <- AncSR1_complete_data %>%
  mutate(padj = p.adjust(p, method = "fdr")) %>%   # recompute adjusted p-values for combined data
  select(-sig) %>%
  mutate(active = padj < 0.1, active = replace(active, is.na(active), FALSE))
...
write.csv(select(AncSR1_complete_data, -(p:padj)), 
          file = gzfile(file.path(results_dir, "AncSR1_complete_data.csv.gz")),
          row.names = FALSE)
write.csv(select(AncSR2_complete_data, -(p:padj)), 
...
# plot number of predicted and observed active variants
AncSR1_complete_data %>%
  filter(active) %>%
  group_by(RE, type) %>%
  dplyr::count() %>%
...
# print number of predicted and observed active variants
...
knitr::kable(mutate(AncSR1_complete_table, prop.pred = round(prop.pred, 3)), 
             caption = "AncSR1 active variants")
# sum across REs
AncSR1_complete_table %>%
  ...
  knitr::kable(caption = "AncSR1 active variants (total)")
...
knitr::kable(mutate(AncSR2_complete_table, prop.pred = round(prop.pred, 3)), 
             caption = "AncSR2 active variants")
# sum across REs
AncSR2_complete_table %>%
  ...
  summarize(observed = sum(observed), predicted = sum(predicted),
            total = sum(total), prop.pred = predicted / total) %>%
  mutate(prop.pred = round(prop.pred, 3)) %>%
  knitr::kable(caption = "AncSR2 active variants (total)")
```


## SRE2 correction

The GA-RE yeast strain used in the sort-seq experiments was modified from the original GA-RE strain to include mutations at nucleotide sites flanking the core SRE2 motif. This was to correct for an artifact in the original GA-RE strain that caused high basal fluorescence in the absence of DBD protein. We expected that these flanking mutations would not affect the binding affinity of SR DBD to the RE site; however, two lines of evidence suggest that the flanking mutations did in fact lower the affinity of the protein for the GA-RE site. First, the wild type AncSR2 protein, which is known to bind and activate gene expression on GA-RE to a similar level as on SRE1 when the flanking sequence is kept constant (McKeown *et al., Cell* 2014; Anderson, McKeown & Thornton, *eLife* 2015), is at the lower bound of fluorescence in our sort-seq assay, whereas wild type AncSR2 on SRE1 is within the dynamic range. Second, we used flow cytometry to measure the fluorescence for a panel of SR DBD variants in the ERE, SRE1, and the modified GA-RE strain, and compared this to *in vitro* affinity measurements for this same panel of SR DBD variants on the same REs, but with the flanking sequence kept constant (McKeown *et al., Cell* 2014; Anderson, McKeown & Thornton, *eLife* 2015). We observed that the affinity-fluorescence relationship appears similar for ERE and SRE1, but is right-shifted for GA-RE, such that fluorescence is lower than expected in SRE2 based on affinity. This is shown below.

```{r affinityfluorescence, purl = FALSE}
# extract mean GFP fluorescence from flow cytometry data on isogenic strains
# measured on ERE, SRE1, and the modified SRE2 (GA)
...
# Plot log10(Kmac) vs. fluorescence for best fit model (SRE2 (GA) effect).
# Solid lines show the best-fit logistic curves with midpoints specific for 
# ERE/SRE1 and SRE2 (GA). Dashed lines show confidence intervals for the ERE/SRE1
# midpoint and SRE2 (GA) midpoint.
...
The above shows that a logistic curve with one midpoint for ERE and SRE1 and a right-shifted midpoint for GA-RE best fits the data, with dashed lines denoting 95% confidence intervals for the midpoints. (The confidence interval is wider for the GA-RE curve because it incorporates uncertainty in both the ERE/SRE1 midpoint and the GA-RE effect.) These observations suggest that the flanking mutations have an affinity-decreasing effect, which causes a global decrease in the fluorescence measured for DBD variants on GA-RE relative to what would have been expected without the flanking mutations.

To correct for this effect, it is not enough to simply adjust all of the GA-RE fluorescence values up because of the nonlinear relationship between affinity and fluorescence. Instead, we can take advantage of the genetic scores inferred from our reference-free models, which account for this nonlinear relationship. Specifically, the effect of the flanking mutations on fluorescence, $\Delta F$, can be expressed either in terms of an effect on affinity, $\Delta\log_{10}(K_A)$, or an effect on genetic score $\Delta x$, *i.e.*
...
\end{align*}
$$
where $a$ is the steepness of the $\log_{10}(K_A)$-fluorescence curve and $\Delta L$ is the difference between the lower bound of the reference-free model and that of the affinity-fluorescence curve. Setting these expressions equal to each other and solving for $\Delta x$, we obtain

$$
\Delta x = -\ln\left(\frac{R}{\Delta L + \frac{R}{1 + e^{-\Delta\log_{10}(K_A) / a}}} - 1\right).
$$

We can thus correct for the effect of the flanking substitutions on GA-RE library variants by taking the inferred effect on affinity, transforming it to a genetic score scale, and subtracting this value from the genetic score inferred for each GA-RE variant to infer corrected fluorescence values for those variants. This is done below using the inferred GA-RE effect on affinity as well as the 95% confidence interval bounds for the effect.
...
# recompute p-values for calling variants active vs. inactive
...
if(!file.exists(file.path(results_dir, "AncSR1_SRE2_corrected_miss_active_p.rda"))) {
  # compute p-value for calling variants active 
  # (greater fluorescence than nonsense variants)
  fstop <- meanF_data %>% 
    filter(type == "exp", bg == "AncSR1", stop == TRUE) %>% 
  ...
  # compute fraction of nonsense variants whose meanF is above that of the test variant;
  # perform test for all non-nonsense variants
  p.AncSR1.SRE2.miss <- sapply(AncSR1.SRE2.missing.fluor.corrected, 
                               function(x) sum(fstop > x) / nstop)
  save(p.AncSR1.SRE2.miss, 
       file = file.path(results_dir, "AncSR1_SRE2_corrected_miss_active_p.rda"))
} else load(file.path(results_dir, "AncSR1_SRE2_corrected_miss_active_p.rda"))
...
if(!file.exists(file.path(results_dir, "AncSR2_SRE2_corrected_miss_active_p.rda"))) {
  # compute p-value for calling variants active 
  # (greater fluorescence than nonsense variants)
  fstop <- meanF_data %>% 
    filter(type == "exp", bg == "AncSR2", stop == TRUE) %>% 
  ...
  # compute fraction of nonsense variants whose meanF is above that of the test variant;
  # perform test for all non-nonsense variants
  p.AncSR2.SRE2.miss <- sapply(AncSR2.SRE2.missing.fluor.corrected, 
                               function(x) sum(fstop > x) / nstop)
  save(p.AncSR2.SRE2.miss, 
       file = file.path(results_dir, "AncSR2_SRE2_corrected_miss_active_p.rda"))
} else load(file.path(results_dir, "AncSR2_SRE2_corrected_miss_active_p.rda"))
...
AncSR1_complete_data_corrected <- AncSR1_model_data %>%
  ...
  bind_rows(mutate(AncSR1_missing_data, type = "predicted",
                   ...
                   p = replace(p, RE == "SRE2 (GA)", p.AncSR1.SRE2.miss))) %>%
  mutate(padj = p.adjust(p, method = "fdr"), active = padj < 0.1, 
         active = replace(active, is.na(active), FALSE)) %>%
  replace_na(list(active = FALSE)) %>%
  select(-sig) %>%
  arrange(AA_var, RE)
AncSR2_complete_data_corrected <- AncSR2_model_data %>%
  ...
  bind_rows(mutate(AncSR2_missing_data, type = "predicted",
                   ...
                   p = replace(p, RE == "SRE2 (GA)", p.AncSR2.SRE2.miss))) %>%
  mutate(padj = p.adjust(p, method = "fdr"), active = padj < 0.1, 
         active = replace(active, is.na(active), FALSE)) %>%
  replace_na(list(active = FALSE)) %>%
  select(-sig) %>%
...
# How many variants active variants with and without the GA-RE correction?
# AncSR1
AncSR1_complete_data_corrected %>%
  group_by(RE) %>%
  summarize(active_corrected = sum(active)) %>%
  left_join(AncSR1_complete_data %>% 
              group_by(RE) %>% 
              summarize(active_uncorrected = sum(active)))
# AncSR2
AncSR2_complete_data_corrected %>%
  group_by(RE) %>%
  summarize(active_corrected = sum(active)) %>%
  left_join(AncSR2_complete_data %>% 
              group_by(RE) %>% 
              summarize(active_uncorrected = sum(active)))
