...
# get predictions for a set of large cross-validated glmnet objects
# glmnet.object is the fit from the training fit (fit to all the data).
# outlist is a list of all the fits from the cross-validation
cv.glmnet.raw.large.predict <- function(x, y, weights, offset, lambda, 
                                        type.measure, nfolds, foldid, alignment, 
                                        grouped, keep, parallel, trace.it, 
                                        glmnet.call, cv.call, glmnet.object, 
                                        outlist, ...) {
  
  # if(trace.it) cat("Training\n")
  # glmnet.object = glmnet.large(x, y, weights = weights, offset = offset,
  ...
  #   outlist = foreach(i = seq(nfolds), .packages = c("glmnet"), .verbose = TRUE) %dopar% {
  #     which = foldid == i
  #     #      if (is.matrix(y))
  #     if(length(dim(y)) > 1)
  #       y_sub = y[!which,]
  ...
  #     fold_fit <- glmnet.large(x[!which, , drop = FALSE], y_sub, lambda = lambda,
  #                              offset = offset_sub, weights = weights[!which], ...)
  #     save(fold_fit, file = file.path(out_dir, paste0("fold", i, "_fit.rda")))
  #     cat(paste("Fold", i, "complete\n"))
  #     fold_fit
  #   }
  # }
  # else {
  ...
  #       offset_sub = as.matrix(offset)[!which,]
  #     else offset_sub = NULL
  #     fold_fit = glmnet.large(x[!which, , drop = FALSE],
  ...
  predmat = buildPredmat(outlist, lambda, x, offset, foldid, alignment, y = y, 
                         weights = weights, grouped = grouped, 
                         type.measure = type.measure, 
                         family = family(glmnet.object))
  ### we include type.measure for the special case of coxnet with the deviance 
  ### vs C-index discrepancy
  ...
  if((N/nfolds < 3) && grouped) {
    warning("Option grouped=FALSE enforced in cv.glmnet, 
            since < 3 observations per fold",
            call. = FALSE)
    grouped = FALSE
  }
  
  out = glmnet:::cvstats(cvstuff, foldid, nfolds, lambda, nz, grouped)
  ...
  lamin = with(out, glmnet:::getOptcv.glmnet(lambda, cvm, cvsd, cvname))
...
# get predictions for a set of large cross-validated glmnet objects
# fullfit is the fit from the training fit (fit to all the data).
# cvfits is a list of all the fits from the cross-validation
cv.glmnet.large.predict <- function(x, y, fullfit, cvfits, weights=NULL, 
                                    offset = NULL, lambda = NULL, type.measure = 
                                      c("default", "mse", "deviance", "class", 
                                        "auc", "mae", "C"), 
  ...
  if(!is.null(lambda) && alignment=="fraction") {
    warning("fraction of path alignment not available if lambda given as argument; 
            switched to alignment=`lambda`")
    alignment = "lambda"
  }
  N = nrow(x)
  if(is.null(weights))
  ...
  if(relax)
    cv.relaxed.raw(x, y, weights, offset, lambda, type.measure, nfolds, foldid, 
                   alignment, grouped, keep, parallel, trace.it, glmnet.call, 
                   cv.call, gamma, ...)
  else
    cv.glmnet.raw.large.predict(x, y, weights, offset, lambda, type.measure, 
                                nfolds, foldid, alignment, grouped, keep, 
                                parallel, trace.it, glmnet.call, cv.call, 
                                glmnet.object = fullfit, outlist = cvfits, ...)
}


# cv.glmnet.raw for large matrices; cross-validation only
# glmnet.object is the fit from the training fit (fit to all the data).
# out_dir is the name of the directory in which to save the individual glmnet 
# fit objects for each fold.
cv.glmnet.raw.large.cv <- function(x, y, weights, offset, lambda, type.measure, 
                                   nfolds, foldid, alignment, grouped, keep, 
                                   parallel, trace.it, glmnet.call, cv.call,
                                   glmnet.object, out_dir, ...) {
...
# cv.glmnet for large matrices, cross-validation only
# glmnet.object is the fit from the training fit (fit to all the data).
# out_dir is the name of the directory in which to save the individual glmnet 
# fit objects for each fold.
cv.glmnet.large.cv <- function(x, y, fullfit, out_dir, weights=NULL, 
                               offset = NULL, lambda = NULL, type.measure = 
                                 c("default", "mse", "deviance", "class", "auc", 
                                   "mae", "C"), 
                               nfolds = 10, foldid=NULL, 
                               alignment = c("lambda", "fraction"), 
                               grouped = TRUE, keep = FALSE, parallel = FALSE, 
                               gamma = c(0, .25, .5, .75, 1), relax = FALSE,
                               trace.it = 0, ...) {
  type.measure = match.arg(type.measure)
  alignment = match.arg(alignment)
  if(!is.null(lambda) && length(lambda) < 2)
  ...
  if(relax)
    ...
    cv.glmnet.raw.large.cv(x, y, weights, offset, lambda, type.measure, nfolds,
                           foldid, alignment, grouped, keep, parallel, trace.it,
                           glmnet.call, cv.call, glmnet.object = fullfit, 
                           out_dir = out_dir, ...)
}


# cv.glmnet.raw for large matrices
cv.glmnet.raw.large <- function(x, y, init, weights, offset, lambda, 
                                type.measure, nfolds, foldid, alignment,
                                grouped, keep, parallel, trace.it, glmnet.call, 
                                cv.call, ...) {
  ...
  if(parallel) {
    ...
    outlist = foreach(i = seq(nfolds), .packages = c("glmnet")) %dopar% {
        ...
        glmnet.large(x[!which, , drop = FALSE], y_sub, init = init, 
    ...
    for(i in seq(nfolds)) {
      ...
      outlist[[i]] = glmnet.large(x[!which, , drop = FALSE], y_sub, init = init, 
                                  lambda = lambda, offset = offset_sub,
                                  weights = weights[!which], 
                                  trace.it = trace.it, ...)
      endtime <- Sys.time()
      print(paste(endtime - starttime))
    }
  }
  lambda = glmnet.object$lambda
  class(outlist) = paste0(subclass, "list")
  ...
  predmat = buildPredmat(outlist, lambda, x, offset, foldid, alignment, y = y, 
                         ...
                         family = family(glmnet.object))
  endtime <- Sys.time()
  print(paste("Predmat", endtime - starttime))
  ### we include type.measure for the special case of coxnet with the deviance 
  ### vs C-index discrepancy
  ### family is included for the new GLM crowd
  ### Next we compute the measures
...
# cv.glmnet for large matrices
cv.glmnet.large <- function(x, y, init = NULL, weights = NULL, offset = NULL, 
                            lambda = NULL, 
  ...
  if(relax)
    ...
    cv.glmnet.raw.large(x, y, init, weights, offset, lambda, type.measure, 
