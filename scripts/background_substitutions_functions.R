# Function for orthogonal nonlinear least squares fitting, modified from the onls
# package. If domain is provided as a two-element vector, it specifies the domain
# of the function to be fitted; optimization is only performed within this domain.
onlsmod <- function(formula, data = parent.frame(), start, jac = NULL, 
                    control = nls.lm.control(), lower = NULL, 
                    upper = NULL, trace = FALSE, subset, weights, na.action, 
                    window = 12, extend = c(0.2, 0.2), domain = NULL, fixed = NULL, 
                    verbose = TRUE, ...) 
{
...
# function for background epistasis model
bgepistasisfn <- function(F1_norm, alpha) {
  1 / (1 + alpha * (1-F1_norm)/F1_norm)
}

# inverse function for background epistasis model
bgepistasisfninv <- function(F2_norm, alpha) {
  1 / (1 + (1-F2_norm)/(alpha*F2_norm))
}
