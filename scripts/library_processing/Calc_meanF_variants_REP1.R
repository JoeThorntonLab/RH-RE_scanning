#!/usr/bin/env Rscript
# Estimate fluorescence for REP1
setwd("/project/joet1/santiagoherrera/DMS_RH_RE/221202_A00639_1438_AHLYYJDRX2-JT-JP-SHA-1s/demultiplexed_reads/merged_dmx_splits/REP1/AA_variant_counts")
library(tidyverse)
...
bind_dfs <- function(names){
  ...
  for(i in 1:length(names)){
    b1 <- paste(names[i],"_BB1_REP1_AA_var_count.csv",sep="")
    b2 <- paste(names[i],"_BB2_REP1_AA_var_count.csv",sep="")
    b3 <- paste(names[i],"_BB3_REP1_AA_var_count.csv",sep="")
    b4 <- paste(names[i],"_BB4_REP1_AA_var_count.csv",sep="")
    ...
    tmp <- full_join(b1_df,b2_df,by=c("AA_var","REBC")) %>% 
      ...
      relocate(REP,.after = Count_total) %>% mutate(REP = replace(REP,is.na(REP),"REP1")) # replace NAs with REP#
    df <- rbind(df,tmp)
  }
  df
}
...
aa_files_REBCs <- unique(gsub("_BB[0-9]_REP1_AA_var_count.csv","",aa_files))
