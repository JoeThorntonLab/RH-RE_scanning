#!/usr/bin/env Rscript
# Estimate fluorescence for REP2
setwd("/project/joet1/santiagoherrera/DMS_RH_RE/221202_A00639_1438_AHLYYJDRX2-JT-JP-SHA-1s/demultiplexed_reads/merged_dmx_splits/REP2/AA_variant_counts")
library(tidyverse)
...
bind_dfs <- function(names){
  ...
  for(i in 1:length(names)){
    b1 <- paste(names[i],"_BB1_REP2_AA_var_count.csv",sep="")
    b2 <- paste(names[i],"_BB2_REP2_AA_var_count.csv",sep="")
    b3 <- paste(names[i],"_BB3_REP2_AA_var_count.csv",sep="")
    b4 <- paste(names[i],"_BB4_REP2_AA_var_count.csv",sep="")
    ...
    tmp <- full_join(b1_df,b2_df,by=c("AA_var","REBC")) %>% 
      ...
      relocate(REP,.after = Count_total) %>% mutate(REP = replace(REP,is.na(REP),"REP2")) # replace NAs with REP#
    df <- rbind(df,tmp)
  }
  df
}
...
bin_Cellcount <- function(bin = "all"){
  ...
  if(bin == 3) c <- 5380470
  if(bin == 4) c <- 3116191
  if(bin == "all") c <- c(71873842,75663838,5380470,3116191)
  c
}
...
aa_files_REBCs <- unique(gsub("_BB[0-9]_REP2_AA_var_count.csv","",aa_files))
