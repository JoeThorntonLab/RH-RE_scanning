#!/usr/bin/env Rscript
# Estimate fluorescence for REP3
setwd("/project/joet1/santiagoherrera/DMS_RH_RE/221202_A00639_1438_AHLYYJDRX2-JT-JP-SHA-1s/demultiplexed_reads/merged_dmx_splits/REP3/AA_variant_counts")
library(tidyverse)
...
bind_dfs <- function(names){
  ...
  for(i in 1:length(names)){
    b1 <- paste(names[i],"_BB1_REP3_AA_var_count.csv",sep="")
    b2 <- paste(names[i],"_BB2_REP3_AA_var_count.csv",sep="")
    b3 <- paste(names[i],"_BB3_REP3_AA_var_count.csv",sep="")
    b4 <- paste(names[i],"_BB4_REP3_AA_var_count.csv",sep="")
    ...
    tmp <- full_join(b1_df,b2_df,by=c("AA_var","REBC")) %>% 
      ...
      relocate(REP,.after = Count_total) %>% mutate(REP = replace(REP,is.na(REP),"REP3")) # replace NAs with REP#
    df <- rbind(df,tmp)
  }
  df
}
...
bin_Cellcount <- function(bin = "all"){
  ...
  if(bin == 3) c <- 7047242
  if(bin == 4) c <- 4303413
  if(bin == "all") c <- c(66662146,87779826,7047242,4303413)
  c
}
...
aa_files_REBCs <- unique(gsub("_BB[0-9]_REP3_AA_var_count.csv","",aa_files))
